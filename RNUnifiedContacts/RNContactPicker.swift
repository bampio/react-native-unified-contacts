...
class PickContactDelegate: NSObject, CNContactPickerDelegate {
    ...
    init(delegate: ContactPickerDelegateDelegate,callback: @escaping (NSArray) -> ()) {
    ...
    func contactPickerDidCancel(_ picker: CNContactPickerViewController) {
        callback( ["User Cancelled",NSNull()] )
        delegate.done()
    }
    
    func contactPicker(_ picker: CNContactPickerViewController, didSelect contact: CNContact) {
        callback([NSNull(), RNContactPicker.contactToDictionary(contact)])
        delegate.done()
    }
}
...
class PickContactsDelegate: NSObject, CNContactPickerDelegate {
    ...
    func contactPicker(_ picker: CNContactPickerViewController, didSelect contacts: [CNContact]) {
        callback( [NSNull(), contacts.map { x in RNContactPicker.contactToDictionary(x) }] )
        delegate.done()
    }
}
